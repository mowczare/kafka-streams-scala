...
class FrequencyStreamTest extends FunSuite with Matchers {
  ...
  test("Sample test") {
    val inputRecords: Seq[(String, InputEvent)] = Seq(
      ("test", InputEvent(4))
    ) ++ ((1 to 100).map(_ => 1) ++ (1 to 100))
      .map(v => v * 2 + 1)
      .map(v => "test" -> InputEvent(v))
    ...
    val streamResult = MockedStreams()
      ...
      .output[Long, ItemSketchWrap[InputEvent]](
        outputTestTopic,
        Serdes.Long,
        SerdeUtil.codecToSerde[ItemSketchWrap[InputEvent]],
        1000
      )

    val finalResult = streamResult
      .groupBy(_._1)
      .mapValues(
        _.last._2.itemSketch.getFrequentItems(ErrorType.NO_FALSE_POSITIVES)
      )
